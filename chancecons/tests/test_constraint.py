...
class TestConstraint(BaseTest):
	...
	def test_properties(self):
		...
		self.assertEqual(ChanceConstraint([self.x >= 0]).max_violations, 0)
		self.assertEqual(ChanceConstraint([self.x >= 0], 0.8).max_violations, (1-0.8)*2)
		self.assertEqual(ChanceConstraint([self.x >= 0, self.y >= 0], 0.8).max_violations, (1-0.8)*(2+3))
		self.assertEqual(ChanceConstraint([self.U >= 0, self.V >= 0], 0.8).max_violations, (1-0.8)*(2*2+3*2))
	...
	def test_restriction(self):
		...
		self.x.value = [-1,1]
		cc.slope.value = 0.5
		val = np.sum(np.maximum(0.5 + self.x.value, 0)) - 0.5*(1-0.8)*2
		...
		cc.fraction = 0.4
		cc.slope.value = 1.5
		val = np.sum(np.maximum(1.5 - self.x.value, 0)) - 1.5*(1-0.4)*2
		self.assertItemsAlmostEqual(cc.restriction.expr.value, val)
		...
		val = np.sum(np.maximum(0.5 + expr, 0)) - 0.5*(1-0.8)*self.A.shape[0]
		self.assertItemsAlmostEqual(cc.restriction.expr.value, val)
	...
	def test_restriction_2d(self):
		...
		self.U.value = np.array([[-1,-2], [3,4]])
		cc.slope.value = 0.5
		val = np.sum(np.maximum(0.5 + self.U.value, 0)) - 0.5*(1-0.8)*(2*2)
		self.assertItemsAlmostEqual(cc.restriction.expr.value, val)
